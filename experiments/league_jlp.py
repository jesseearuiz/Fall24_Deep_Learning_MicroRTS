...
from cma_autoenc_transformer import Agent as jlp_agent
...
class Match:
    def __init__(self, partial_obs: bool, match_up=None, map_path="maps/16x16/basesWorkers16x16A.xml"):
        ...
        if mode == 0:
            ...
            self.agent = jlp_agent(self.envs).to(self.device)
            self.agent.load_state_dict(torch.load(self.rl_ai, map_location=self.device,weights_only=False))
            self.agent.eval()
        elif mode == 1:
            self.envs = MicroRTSGridModeVecEnv(
                num_bot_envs=0,
    ...
    def run_m1(self, num_matches):
        ...
        while True:
            ...
            with torch.no_grad():
                ...
                p1_mask = mask[::2]
                p2_mask = mask[1::2]
                #Edit JLP agent logic here
                p1_action, _, _, _, _ = self.agent1.get_action_and_value(
