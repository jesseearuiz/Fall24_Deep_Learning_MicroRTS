...
def parse_args():
    ...
    parser.add_argument('--num-bot-envs', type=int, default=8,
...
class Strategy(nn.Module):
    def __init__(self, hidden_size, enc_layers, dec_layers, enc_heads, dec_heads, context_size=4,batch_size=1):
        super(Strategy, self).__init__()
        self.batch_size = batch_size
        self.hidden_size = hidden_size
        #state embedding
        self.state_embed = nn.Embedding(7200, hidden_size//2)
        #action embedding
        self.action_embed = nn.Embedding(256 * 6 * 7, hidden_size//2)
        #encoder takes in embedded state and embedded action
        ...
        #decoder takes in the context of the previous state,action pair and trys to predict the next state
        self.decoder = nn.TransformerDecoder(
            nn.TransformerDecoderLayer(d_model=hidden_size, nhead=4,
                dim_feedforward= hidden_size * 4,
                batch_first=True,
                ),
    ...
    def forward(self, action, state, device="cpu"):
        state = self.state_embed(state)
        action = self.action_embed(action)
        in_data = torch.cat((state,action),dim=-1).to(device)
        memory = self.encoder(in_data)
        tgt = torch.zeros_like(memory).to(device)
        tgt[0] = memory[0]
        predicted_next_states = self.decoder.forward(tgt, memory)
        return memory, predicted_next_states
...
class Agent(nn.Module):
    def __init__(self, envs, mapsize=16 * 16):
        ...
        self.hidden_size = 16
        #world model takes in the previous state and action * context_size
        #and outputs the next context_size states
        self.world_model = Strategy(hidden_size=self.hidden_size, enc_layers=3, dec_layers=3, enc_heads=6, dec_heads=6, context_size=4, batch_size=envs.num_envs)
        #policy takes in the the context_size num states and outputs the next action for each square
        self.policy = nn.TransformerDecoder(
            nn.TransformerDecoderLayer(d_model=self.hidden_size, nhead=4,
                dim_feedforward=self.hidden_size * 4,
                batch_first=True,
                ),
            num_layers=2,
        )
        self.unembed = layer_init(nn.Linear(self.hidden_size, 78))
        self.value = nn.Sequential(
            layer_init(nn.Linear(self.hidden_size, self.hidden_size)), nn.ReLU(), layer_init(nn.Linear(self.hidden_size, 1))
            )
    ...
    def get_action_and_value(self, x, action=None, invalid_action_masks=None, envs=None, device=None):
        ...
        x = mrts_utils.compact_observation_space(x).prod(dim=-1).to(device)
        if action != None:
          action = action.argmax(dim=-1).int().to(device)
        else:
          action = torch.zeros(x.shape[0],self.mapsize,7,dtype=torch.int32).prod(dim=-1)
        context, predicted_future_states = self.world_model(action, x,device=device)
        value = self.value(context).sum(dim=1)
        prompt = torch.zeros((x.shape[0],self.mapsize,self.hidden_size)).to(device)
        self.policy(prompt, predicted_future_states)
        logits = self.unembed(self.policy(prompt, predicted_future_states))
    ...
    def get_value(self, x):
        x = x.flatten(start_dim=1, end_dim=2)
        x = mrts_utils.compact_observation_space(x).prod(dim=-1)
        action = torch.zeros(x.shape[0],self.mapsize,78,dtype=torch.int32).prod(dim=-1).to(device)
        context, predicted_future_states = self.world_model(action, x,device=device)
        value = self.value(context).sum(dim=1)
        return value
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                action, logproba, _, _, vs = agent.get_action_and_value(
                    next_obs, envs=envs, action=actions[step-1],invalid_action_masks=invalid_action_masks[step], device=device
                )
