...
class Strategy(nn.Module):
    def __init__(self, hidden_size, enc_layers, dec_layers, enc_heads, dec_heads):
        super(Strategy, self).__init__()
        self.state_embed = nn.Linear(6, hidden_size)
        self.hidden_size = hidden_size
        #action embedding
        #encoder takes in embedded state and embedded action
        self.encoder = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=hidden_size, nhead=enc_heads, dim_feedforward=hidden_size * 4),
            num_layers=enc_layers,
        )
        #decoder takes in the context of the previous state,action pair and trys to predict the next state
        self.decoder = nn.TransformerDecoder(
            nn.TransformerDecoderLayer(d_model=hidden_size, nhead=dec_heads,
    ...
    def forward(self, state):
        state = self.state_embed(F.relu(state.reshape(-1,6)))
        memory = self.encoder(state)
        tgt = torch.zeros_like(memory)
        action_embeddings = self.decoder.forward(tgt, memory)
        return memory, action_embeddings
...
class Agent(nn.Module):
    def __init__(self, envs, mapsize=16 * 16):
        ...
        self.world_model = Strategy(hidden_size=self.hidden_size, enc_layers=2, dec_layers=2, enc_heads=4, dec_heads=4)
        #policy takes in the the context_size num states and outputs the next action for each square
        self.policy = nn.Sequential(
            nn.Linear(self.hidden_size, self.hidden_size), nn.ReLU(), nn.Linear(self.hidden_size, 78)
            )
        self.value = nn.Sequential(
            nn.Linear(self.hidden_size, self.hidden_size), nn.ReLU(), nn.Linear(self.hidden_size, 1)
            )
    ...
    def get_action_and_value(self, x, action=None, invalid_action_masks=None, envs=None, device=None):
        ...
        x = mrts_utils.compact_observation_space(x).float()
        context, action = self.world_model(x)
        value = self.value(context.reshape(-1, self.hidden_size)).reshape(-1, self.mapsize).sum(dim=1)
        logits = self.policy(action.reshape(-1, self.hidden_size)).reshape(-1, self.mapsize, 78)
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                action, logproba, _, _, vs = agent.get_action_and_value(
                    next_obs, envs=envs, action=None,invalid_action_masks=invalid_action_masks[step], device=device
                )
