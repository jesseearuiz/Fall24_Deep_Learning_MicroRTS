...
class JLPAgent(nn.Module):
    def __init__(self, envs, mapsize=16 * 16):
        super(JLPAgent, self).__init__()
        ...
        self.hidden_ff = nn.Sequential(nn.Linear(1024, 2048),nn.ReLU(),nn.Linear(2048, 1024),nn.ReLU())
    ...
    def get_action_and_value(self, x, action=None, invalid_action_masks=None, envs=None, device=None):
        ...
        hidden = self.hidden_ff(hidden.reshape(-1,1024)).reshape(-1, 64, 4, 4)
